...
def train(args):
    ...
    #                               betas=(0.9, 0.999), weight_decay=0)##weight-decay 0.01
    # scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=20, gamma=0.95)
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.init_lr, eps=1e-8,
                                 betas=(0.9, 0.999), weight_decay=0)##weight-decay 0.01
    WARMUP_LR_TIMES = 1.0
    WARMUP_EPOCH = 10
    ...
    for epoch in range(args.n_epoch + 1):
        ...
        if epoch>=100:
            ...
            if best_acc == (np.sum((results[:,0] <= 0.050) * (results[:,1] <= 5)) * 1. / len(results) * 100) or \
                ...
                print("hello")
                root_path = Path("/mnt/share/sda1/modelghb/12s/")
